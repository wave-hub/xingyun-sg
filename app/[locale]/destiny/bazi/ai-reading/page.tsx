...
import { Compass, CheckCircle, CreditCard, Brain, Sparkles } from "lucide-react";
...
const plans = [
  { id: "quick", name: "AI 快速解讀", price: "S$28", desc: "即時 AI 生成基礎報告", features: ["四柱排定", "五行分析", "日主判定"] },
  { id: "full", name: "AI 完整解讀", price: "S$68", desc: "AI 深度分析完整報告", features: ["包含快速版所有內容", "大運流年", "五行調和建議"], popular: true },
  { id: "master", name: "人工深度解讀", price: "S$288", desc: "大師一對一諮詢", features: ["包含完整版所有內容", "擇日合婚", "大師建議"], popular: false },
];

export default function BaziAiReadingPage() {
      ...
      <section className="py-12 px-4">
        <div className="container-brand max-w-4xl mx-auto text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-neutral-400">三步獲得專業八字分析報告</p>
          </motion.div>
        </div>
      </section>
                ...
                <Card variant="glass" className="p-6 mb-6">
                  <CardContent>
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between text-sm"><span className="text-neutral-400">出生日期</span><span className="text-ivory">{birthInfo?.birthDate}</span></div>
                      <div className="flex justify-between text-sm"><span className="text-neutral-400">性別</span><span className="text-ivory">{birthInfo?.gender === "male" ? "男" : "女"}</span></div>
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
              {isGenerating ? (
                <div className="text-center py-20">
                  ...
                  <h2 className="text-xl font-serif-tc font-bold text-ivory mb-2">AI 正在為您生成八字分析...</h2>
                  <p className="text-sm text-neutral-400">五行運算中，請稍候片刻</p>
                </div>
              ) : showReport ? (
                <ReadingReport report={{ ...mockReport, title: "八字命理解讀報告", titleEn: "Ba Zi Reading Report", method: "八字命理", methodEn: "Ba Zi" }} />
              ) : null}
            </motion.div>
          )}
        </div>
